...
export class ClassPanel extends React.Component {
    ...
    */
    DateButtons(e) {
        ...
        for (let i = 0; i < 7; i++) {
            if (findDate(this.state.Class_Detail[e].Date, days[i])) {
                bColor = "#ff0000";
            } else {
                bColor = "";
            }
...
//need to check later                
            case "LocationB":
                this.state.Class_Detail[selectedIndex].LocationB = value;
                this.setState({
    ...
    handlePreProcess() {
        this.state.ClassName = this.state.CourseSubject + ' ' + this.state.CourseCode;
    }
    ...
    handleSelectChange(event, selectedIndex) {
        ...
        switch (name) {
            case "StartHour":
                this.state.Class_Detail[selectedIndex].TimeFrom.hour(value);
                ...
                break;
            case "StartMint":
                this.state.Class_Detail[selectedIndex].TimeFrom.minute(value);
                ...
                break;
            case "StopHour":
                this.state.Class_Detail[selectedIndex].TimeTo.hour(value);
                ...
                break;
            case "StopMint":
                this.state.Class_Detail[selectedIndex].TimeTo.minute(value);
    ...
    newTabs() {
        ...
        for (let i = 0; i < this.state.Class_Detail.length; i++) {
            if (this.state.Class_Detail[i].Types === type1) {
                content = "LEC " + count1;
                count1++;
            } else if (this.state.Class_Detail[i].Types === type2) {
                content = "TUT " + count2;
                count2++;
            } else if (this.state.Class_Detail[i].Types === type3) {
                content = "LAB " + count3;
                count3++;
            } else {
                content = "Time " + (i + 1);
                ...
                React.createElement("div", {className: "stopTime"},
                    ...
                    React.createElement("div", {className: "StopTimeInput"},
                        <select className="inputs" id="StopHour" name="StopHour" size="0"
                                value={this.state.TimeTo.hour()}
                                onChange={(event) => this.handleSelectChange(event, i)}>
                            {ClassPanel.Hours()}
                        </select>,
                        <select className="inputs" id="StopMint" name="StopMint" size="0"
                                value={this.state.TimeTo.minute()}
                                onChange={(event) => this.handleSelectChange(event, i)}>
                            {ClassPanel.Mints()}
                        </select>
                    )),
